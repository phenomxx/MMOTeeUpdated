...
{
	if(ClientID >= MAX_CLIENTS || m_aClients[ClientID].m_State == CClient::STATE_EMPTY)
	{
		...
		try
		{
			if(bGetCount)
			{
				str_format(aBuf, sizeof(aBuf),
					...
					"WHERE item_owner = '%d' AND item_type != '10, 12, 15, 16, 17, 18, 19';",
			...
			while(pSqlServer->GetResults()->next())
			{
				...
				if(m_Type == 15 || m_Type == 16 || m_Type == 17 || m_Type == 18 || m_Type == 19)
				{
					str_format(iUsed, sizeof(iUsed), "it%d", ItemID);
					if (m_Type != 19) {
						str_format(iName, sizeof(iName), "➳ Lvl%d %s +%d",
							m_pServer->GetItemPrice(m_ClientID, ItemID, 0), m_pServer->GetItemName(m_ClientID, ItemID), m_pServer->GetItemEnchant(m_ClientID, ItemID));
					}
					else {
						str_format(iName, sizeof(iName), "➳ Lvl%d %s",
							m_pServer->GetItemPrice(m_ClientID, ItemID, 0), m_pServer->GetItemName(m_ClientID, ItemID));
					}
