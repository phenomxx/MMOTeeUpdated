...
/* INFECTION MODIFICATION START ***************************************/
	...
	if (GetClass() == PLAYERCLASS_ASSASINS) {
		if (m_QueuedWeapon == WEAPON_HAMMER)
			GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Current weapon: hammer");
		if (m_QueuedWeapon == WEAPON_KNIFE)
			GameServer()->SendChatTarget(m_pPlayer->GetCID(), "Current weapon: knife");
	}
...
void CCharacter::FireWeapon()
{
	...
	if((m_ActiveWeapon == WEAPON_HAMMER && Server()->GetItemSettings(m_pPlayer->GetCID(), HAMMERAUTO)) ||
		...
		(m_ActiveWeapon == WEAPON_HAMMER && m_BerserkRage))
		FullAuto = true;
	...
	switch(m_ActiveWeapon)
	{
		case WEAPON_HAMMER:
		{
			...
			for (CPixi* pPixi = (CPixi*)GameWorld()->FindFirst(CGameWorld::ENTTYPE_PIXI); pPixi; pPixi = (CPixi*)pPixi->TypeNext())
			{
	...
	if (m_ActiveWeapon >= WEAPON_AXE && m_ActiveWeapon <= WEAPON_WAND) {
		// Определяем радиус
		int Range = 0;
		if (m_pPlayer->AccData.Class == PLAYERCLASS_BERSERK)	Range = m_pPlayer->AccUpgrade.HammerRange * 20;
		else if (m_pPlayer->AccData.Class == PLAYERCLASS_ASSASINS) Range = 100;
		...
		// Magic епта
		if (m_ActiveWeapon == WEAPON_AXE) {
			if (Server()->Tick() >= m_ReloadActiveTick) {
				m_BerserkRage = 1;
				m_BerserkRageEndTick = Server()->Tick() + Server()->TickSpeed() * 5;
				m_ReloadActiveTick = Server()->Tick() + Server()->TickSpeed() * 60;
			}
		}

		if (m_ActiveWeapon == WEAPON_KNIFE) {
			if (Server()->Tick() >= m_ReloadActiveTick) {
				m_InvisibleEndTick = Server()->Tick() + Server()->TickSpeed() * 5;
...
void CCharacter::Tick()
{
	...
	// Берсерк ярость
	if (m_BerserkRage) {
		if (m_pPlayer->m_TeeInfos.m_ColorBody != 65361)
			m_OldPlayerBodyColor = m_pPlayer->m_TeeInfos.m_ColorBody;
		m_pPlayer->m_TeeInfos.m_ColorBody = 65361;
	}
	else
		if (m_OldPlayerBodyColor != 0)
			m_pPlayer->m_TeeInfos.m_ColorBody = m_OldPlayerBodyColor;
	if (Server()->Tick() >= m_BerserkRageEndTick)
		m_BerserkRage = 0;

	// Хиллер медитация
	if (m_Core.m_Vel <= vec2(1, 1) && GetClass() == PLAYERCLASS_HEALER)
		m_DontMoveTick++;
	else
		m_DontMoveTick = 0;
	if (Server()->Tick() % Server()->TickSpeed() == 0) {
		if (m_DontMoveTick >= Server()->TickSpeed() * 3) {
			GameServer()->CreateDeath(m_Pos, m_pPlayer->GetCID());
	...
	// Отсосин инвиз
	if (!(Server()->Tick() % Server()->TickSpeed()) && m_InvisibleEndTick > Server()->Tick()) {
		int SecLeft = (m_InvisibleEndTick - Server()->Tick()) / Server()->TickSpeed();
		GameServer()->SendChatTarget_Localization(m_pPlayer->GetCID(), CHATCATEGORY_DEFAULT, _("The invisibility effect will end in {int:time} sec"), "time", &SecLeft);
	}
	...
	if (m_pPlayer->m_Tele) {
		if (GameServer()->m_apPlayers[m_pPlayer->m_GrabbedID]) {
			if (GameServer()->m_apPlayers[m_pPlayer->m_GrabbedID]->GetCharacter()) {
				GameServer()->m_apPlayers[m_pPlayer->m_GrabbedID]->GetCharacter()->m_Core.m_Pos = m_Core.m_Pos + vec2(m_Input.m_TargetX, m_Input.m_TargetY);
				GameServer()->m_apPlayers[m_pPlayer->m_GrabbedID]->GetCharacter()->m_Core.m_Vel = vec2(0, 0);
			}
		}
		else {
			m_pPlayer->m_Tele = 0;
	...
	if(pFrom && pChr)
	{
		...
		if(!m_pPlayer->m_InArea && !pFrom->m_InArea)
		{
			...
			if(m_pPlayer->GetBotType() == BOT_NPC)
			{
				...
				GameServer()->SendChatTarget_Localization(From, CHATCATEGORY_DEFAULT, _("Relations angry: {int:rel}"), "rel", &pFrom->AccData.Rel, NULL);
			}
		}
		// Арена
	...
	if(From >= 0 && pFrom && pFrom->GetCharacter())
	{
		...
		else
		{
			...
			if(pFrom->GetCharacter()->m_BerserkRage && Weapon == WEAPON_HAMMER)
				DamageProc += DamageProc / 2;

			// -30% урона если ты красный как залупа
			if(m_BerserkRage)
				DamageProc -= (DamageProc / 100) * 30;
			...
			for(int i = 0; i < MAX_PLAYERS; ++i)
			{
				...
				if(GameServer()->m_apPlayers[i] && GameServer()->m_apPlayers[i]->GetCharacter())
				{
					if(GameServer()->m_apPlayers[i]->m_InBossed)
					{
						...
						if(!g_Config.m_SvCityStart)
						{
							CreateDropRandom(MONEYBAG, 40+rand()%5+1, false, i, Force/(50+randforce));
						...
						{
							CreateDropRandom(MONEYBAG, 70+rand()%20+1, false, i, Force/(50+randforce));
...
void CCharacter::Snap(int SnappingClient)
{
	...
	if (NetworkClipped(SnappingClient))
		return;
	if (Server()->Tick() <= m_InvisibleEndTick && SnappingClient != m_pPlayer->GetCID())
		return;
