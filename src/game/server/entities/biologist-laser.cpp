...
bool CBiologistLaser::HitCharacter(vec2 From, vec2 To)
{
	...
	if (GameServer()->m_apPlayers[m_Owner]->m_InArea
		&& GameServer()->m_AreaType == 2 && !GameServer()->m_AreaStartTick)
		pHit->Freeze(10);
	else
	{
		if (m_Explode)
...
void CBiologistLaser::DoBounce()
{
	...
	if (m_Energy < 0 || !GameServer()->m_apPlayers[m_Owner] || !GameServer()->GetPlayerChar(m_Owner))
	{
