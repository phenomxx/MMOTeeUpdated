				...
				if (m_apPlayers[i]->GetCharacter())
					if (Server()->GetItemEnquip(i, 18) == PET_CLEVER)
						if (m_apPlayers[i]->m_Health < m_apPlayers[i]->m_HealthStart)
							m_apPlayers[i]->GetCharacter()->m_Health += 250;
	}
	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		else if(MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			else
			{
				...
				{
					SendChatTarget(ClientID, "Check /rules");
					return;
				}
...
void CGameContext::ResetVotes(int ClientID, int Type)
{
	...
	else if(Type == ARMORMENU)
	{
		...
		AddVote_Localization(ClientID, "armor5", "☞ Wings {str:enquip}", "enquip", Server()->GetItemName(ClientID, Server()->GetItemEnquip(ClientID, 19)));
		...
		else if (m_apPlayers[ClientID]->m_SelectArmor == 5)
			Server()->ListInventory(ClientID, 19);
	}
	...
	else if(Type == INTITLE)
	{
		...
		CreateNewShop(ClientID, PREMIUM_GOVNO, 1, 0, 0);
	...
	else if(Type == RULEMENU)
	{
		...
		AddVote_Localization(ClientID, "null", "SERVER UPDATE v2.0 (New life of server)");
		AddVote("", "null", ClientID);
		AddVote_Localization(ClientID, "null", "Added wings");
		AddVote_Localization(ClientID, "null", "Added actived skills");
		AddVote("", "null", ClientID);
		AddBack(ClientID);
		return;
	}
	...
	else if(Type == CDONATE)
	{
		...
		AddVote_Localization(ClientID, "null", "For buy say phenomxx");
		...
		AddVote_Localization(ClientID, "bprem", "☞ Premium Package [1200]");
		AddVote_Localization(ClientID, "null", "SP Box, 20.000 Money Bag");
		AddVote_Localization(ClientID, "null", "X2 - Money & Exp + Special Snap Draw Item");
		AddVote("", "null", ClientID);
		AddBack(ClientID);
		return;
	...
	else if(Type == CRAFTING)
	{
		...
		if(m_apPlayers[ClientID]->GetCharacter() && m_apPlayers[ClientID]->GetCharacter()->InCrafted())
		{
			...
			{
				//AddNewCraftVote(ClientID, "Rare fish x20, Potato x10, Common fish x50", FISH_SOUP);
				//AddNewCraftVote(ClientID, "Carrot x20, Potato x10, Tomato x15", VEGATIABLES_SOUP);
				//AddNewCraftVote(ClientID, "Carrot x30, Potato x15, Common fish x20", SALAD);
			}
...
void CGameContext::UseItem(int ClientID, int ItemID, int Count, int Type)
{
	...
	if(Type == USEDUSE)
	{
		...
		for(int i = 0; i < Count; ++i)
		{
			...
			else if (ItemID == PREMIUM_BOX)
			{
				Count = 1;
				SendMail(ClientID, "You buying Premium your bonus!", SKILLUPBOX, 1);
				SendMail(ClientID, "You buying Premium your bonus!", PREMIUM_GOVNO, 1);
				SendMail(ClientID, "You buying Premium your bonus!", MONEYBAG, 20000);
				UpdateStats(ClientID);
				break;
			}
