...
#include <game/server/entities/drawpremiumhyeta.h>
#include <game/server/entities/pixi.h>
#include <game/server/entities/wings.h>
#include <time.h>
#include <iostream>
...
bool CGameControllerMOD::OnEntity(const char* pName, vec2 Pivot, vec2 P0, vec2 P1, vec2 P2, vec2 P3, int PosEnv)
{
	...
	else if (str_comp(pName, "Mushroom") == 0)
	{
		vec2 Pos = (P0 + P1 + P2 + P3) / 4.0f;
		new CPickup(&GameServer()->m_World, 0, Pos, 7);
	}
	else if (str_comp(pName, "Pixi1Type") == 0)
	{
		vec2 Pos = (P0 + P1 + P2 + P3) / 4.0f;
		new CPixi(&GameServer()->m_World, Pos);
	}
	else if (str_comp(pName, "PixiBarrier") == 0)
	{
		vec2 Pos = (P0 + P1 + P2 + P3) / 4.0f;
		GameServer()->m_PixiBarriers.push_back(Pos);
	}
...
bool CGameControllerMOD::PreSpawn(CPlayer* pPlayer, vec2 *pOutPos)
{
	...
	if (m_SpawnPoints[Type].size() != 0) {
		int RandomShift = random_int(0, m_SpawnPoints[Type].size());
